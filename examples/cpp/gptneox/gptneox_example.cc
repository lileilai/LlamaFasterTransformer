...
int main(int argc, char* argv[])
{
    ...
    if (data_type == "fp32") {
        gptneox_example<float>(reader);
    }
    else if (data_type == "fp16") {
        gptneox_example<half>(reader);
    }
    else {
...
void gptneox_example(const INIReader reader)
{
    ...
    int int8_mode = reader.GetInteger("ft_instance_hyperparameter", "int8_mode");
    ...
    fastertransformer::GptNeoXWeight<T> gpt_weights(hidden_units,
                                                    inter_size,
                                                    vocab_size,
                                                    decoder_layers,
