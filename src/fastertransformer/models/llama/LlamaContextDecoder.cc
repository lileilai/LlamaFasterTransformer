...
void LlamaContextDecoder<T>::initialize()
{
    ...
    ffn_layer_ = new TensorParallelSiluFfnLayer<T>(static_cast<size_t>(0),  // max_batch_size
                                                   static_cast<size_t>(0),  // max_seq_len
                                                   head_num_,
                                                   size_per_head_,
                                                   static_cast<size_t>(0),  // expert_num
                                                   inter_size_,
                                                   tensor_para_,
                                                   stream_,
                                                   cublas_wrapper_,
                                                   ...
                                                   is_free_buffer_after_forward_,
                                                   false,
                                                   true,  // use_gated_activation = true for llama;
                                                   custom_all_reduce_comm_,
                                                   enable_custom_all_reduce_);
}
                                       ...
                                       const std::vector<LlamaDecoderLayerWeight<T>*>* gpt_decoder_layer_weight)
{
    ...
    AttentionType attention_type  = (d_prefix_prompt_lengths != nullptr || is_qk_buf_float_ || int8_mode_==2) ?
                                        getUnfusedAttentionType(attention_type_) :
                                        attention_type_;
    const bool    is_unpadded_mha = isUnPaddedMHA(attention_type);
    std::cout<<"iteration_num: "<<iteration_num<<", local_batch_size: "<<local_batch_size<<std::endl;
    for (int ite = 0; ite < iteration_num; ite++) {
        ...
        for (int l = 0; l < num_layer_; l++) {
            ...
            // Tensor test{MEMORY_GPU, data_type, {h_token_num, (size_t)hidden_units_}, layer_input};
            ...
            // std::cout<<self_attention_output_tensors.at("hidden_features").toString()<<std::endl;


            if (is_final == false) {
                // 不同于gpt_neox的use_parallel_residual的计算逻辑
                 invokeGeneralAddBiasResidualT5PreLayerNorm(self_attn_output_,
                                                   ...
                                                   (const T*)nullptr, // for llama post_layer_norm.bias
                                                   (const T*)nullptr, // for llama attention_out.bias
                                                   layernorm_eps_,
                                                   h_token_num,
                                                   hidden_units_,
                                                   stream_);
