    ...
    prompt_learning_pair_(prompt_learning_pair)
{
    ...
    for (int l = 0; l < num_layer_; l++) {
        std::cout<<"isValidLayerParallelId: "<<isValidLayerParallelId(l)<<" , layer: "<<l<<std::endl;
...
void LlamaWeight<T>::loadModel(std::string dir_path)
{
    ...
    std::cout<<"start load bins from folder"<<std::endl;
    loadWeightFromBin<T>(
        weights_ptr[0], {(size_t)(vocab_size_ * hidden_units_)}, dir_path + "/model.wte.bin", model_file_type);
    loadWeightFromBin<T>(
        weights_ptr[1], {(size_t)hidden_units_}, dir_path + "/model.norm.bias.bin", model_file_type);
    loadWeightFromBin<T>(
        weights_ptr[2], {(size_t)hidden_units_}, dir_path + "/model.norm.weight.bin", model_file_type);
