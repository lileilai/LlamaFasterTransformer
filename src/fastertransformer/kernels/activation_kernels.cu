...
template<template<typename T> class Activation, typename T, typename BT>
__global__ void llama_generic_activation(T*                      out,
                                   const BT*  __restrict   bias,
                                   const T*   __restrict   gated_weights,
                                   const BT*  __restrict   gated_bias,
                                   const int* __restrict   ia3_tasks,
                                   ...
                                   int n)
{
    ...
    for (int id = blockIdx.x * blockDim.x + threadIdx.x; id < m * n; id += blockDim.x * gridDim.x) {
        ...
        if (with_gate) {
            val = cuda_cast<T>(Activation<T>::apply(gated_val) * cuda_cast<Act_T>(val));
        }
        else {
            val = cuda_cast<T>(Activation<T>::apply(val));
        }
...
template<template<typename T> class Activation, typename T, typename BT>
void invokeLlamaGenericActivation(T*           out,
                             const BT*    bias,
                             const T*     gated_weights,
                             const BT*    gated_bias,
                             const int*   ia3_tasks,
                             ...
                             cudaStream_t stream)
{
    ...
    llama_generic_activation<Activation><<<grid, block, 0, stream>>>(reinterpret_cast<PT*>(out),
                                                               reinterpret_cast<const PBT*>(bias),
                                                               reinterpret_cast<const PT*>(gated_weights),
                                                               reinterpret_cast<const PBT*>(gated_bias),
                                                               ia3_tasks,
...
#define INSTANTIATE_LLAMA_GENERIC_ACTIVATION(Activation, T, BT)                                                              \
    template void invokeLlamaGenericActivation<Activation, T, BT>(T * out,                                                  \
                                                             const BT*    bias,                                        \
                                                             const T*     gated_weights,                               \
                                                             const BT*    gated_bias,                                  \
                                                             const int*   ia3_tasks,                                   \
...
INSTANTIATE_LLAMA_GENERIC_ACTIVATION(SiluActivation, float, float);
INSTANTIATE_LLAMA_GENERIC_ACTIVATION(SiluActivation, half, half);
#ifdef ENABLE_BF16
INSTANTIATE_LLAMA_GENERIC_ACTIVATION(SiluActivation, __nv_bfloat16, __nv_bfloat16);
#endif
