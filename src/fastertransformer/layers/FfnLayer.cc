...
void FfnLayer<T>::forward(TensorMap* output_tensors, TensorMap* input_tensors, const FfnWeight<T>* ffn_weights)
{
    ...
    if (int8_mode_ != 1 || ia3_tasks != nullptr || use_gated_activation) {
        ...
        // std::cout<<"use gated activation: "<< use_gated_activation<<std::endl;
        genericActivation(m,
                          nullptr,
...
#define INVOKE_LLAMA_GENERIC_ACT(ACT)                                                                                        \
    invokeLlamaGenericActivation<ACT>(inter_buf_,                                                                           \
                                 bias1,                                                                                \
                                 inter_buf_2_,                                                                         \
                                 bias2,                                                                                \
                                 ia3_tasks,                                                                            \
                                    ...
                                    const int    seq_len)
{
    ...
    switch (getActivationType()) {
        ...
        case ActivationType::Silu:
            // add this case for llama inference
            INVOKE_LLAMA_GENERIC_ACT(SiluActivation);
            break;
...
// template<typename T>
// LlamaGeluFfnLayer<T>::LlamaGeluFfnLayer(size_t           max_batch_size,
//                               size_t           max_seq_len,
//                               size_t           head_num,
//                               size_t           size_per_head,
//                               size_t           expert_num,
...
// template<typename T>
// LlamaGeluFfnLayer<T>::LlamaGeluFfnLayer(LlamaGeluFfnLayer<T> const& llama_gelu_ffn_layer): FfnLayer<T>(llama_gelu_ffn_layer)
// {
// }


// template<typename T>
// void LlamaGeluFfnLayer<T>::forward(std::vector<fastertransformer::Tensor>*       output_tensors,
//                           const std::vector<fastertransformer::Tensor>* input_tensors,
//                           const LlamaFfnWeight<T>*                           ffn_weights)
// {
//     TensorMap input_tensor({{"ffn_input", input_tensors->at(0)}});
//     TensorMap output_tensor({{"ffn_output", output_tensors->at(0)}});
//     forward(&output_tensor, &input_tensor, ffn_weights);
// }
// template<typename T>
// void LlamaGeluFfnLayer<T>::forward(TensorMap* output_tensors, TensorMap* input_tensors, const LlamaFfnWeight<T>* ffn_weights)
// {
...
//                                   m,
//                                   hidden_units_,
//                                   ffn_weights->gate_weight.kernel,
//                                   inter_size_,
//                                   input_tensor,
//                                   hidden_units_,
//                                   inter_buf_,
...
//     // torchMul， 复用up_proj_buf_
//     torchMul(inter_buf_, up_proj_buf_, up_proj_buf_, m * inter_size_);
...
//     if (is_free_buffer_after_forward_ == true) {
//         FfnLayer<T>::freeBuffer();
//     }
//     sync_check_cuda_error();

// }

// template class LlamaGeluFfnLayer<float>;
// template class LlamaGeluFfnLayer<half>;
// #ifdef ENABLE_BF16
// template class LlamaGeluFfnLayer<__nv_bfloat16>;
// #endif
