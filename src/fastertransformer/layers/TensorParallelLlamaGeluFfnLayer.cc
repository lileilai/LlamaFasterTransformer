...
#include "src/fastertransformer/layers/TensorParallelLlamaGeluFfnLayer.h"
#include "src/fastertransformer/utils/nvtx_utils.h"

namespace fastertransformer {

template<typename T>
void TensorParallelLlamaGeluFfnLayer<T>::forward(std::vector<fastertransformer::Tensor>*       output_tensors,
...
template<typename T>
void TensorParallelLlamaGeluFfnLayer<T>::forward(TensorMap*          output_tensors,
                                            ...
                                            const LlamaFfnWeight<T>* ffn_weights)
{
    ...
    LlamaGeluFfnLayer<T>::forward(output_tensors, input_tensors, ffn_weights);
...
TensorParallelGeluFfnLayer<T>::TensorParallelGeluFfnLayer(size_t           max_batch_size,
    ...
    LlamaGeluFfnLayer<T>(max_batch_size,
                    max_seq_len,
                    head_num,
                    size_per_head,
                    expert_num,
...
template<typename T>
TensorParallelLlamaGeluFfnLayer<T>::TensorParallelLlamaGeluFfnLayer(TensorParallelLlamaGeluFfnLayer<T> const& ffn_layer):
    LlamaGeluFfnLayer<T>(ffn_layer),
...
template class TensorParallelLlamaGeluFfnLayer<float>;
template class  TensorParallelLlamaGeluFfnLayer<half>;
#ifdef ENABLE_BF16
template class TensorParallelLlamaGeluFfnLayer<__nv_bfloat16>;
#endif
